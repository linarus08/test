...
def get_files(dir_name):
    ...
    for root, dirs, files in os.walk(dir_name):
        list_files: list[LiteralString | str | bytes] = [os.path.join(root, file) for file in files]
    return list_files
    ...
    with pd.ExcelWriter(new_file, engine='openpyxl') as writer:
        ...
        for i in range(2, len(df) + 2):
            ...
            worksheet[f"M{i}"].fill = PatternFill(start_color="FFFF00", end_color="FFFF00",
                                                  fill_type="solid")
            worksheet[f"N{i}"] = f"=IF(ROUND(H{i}-G{i}, 2)=M{i}, 0, L{i}-H{i})"
            ...
            worksheet[f"U{i}"].fill = PatternFill(start_color="FFFF00", end_color="FFFF00",
                                                  fill_type="solid")
            worksheet[f"V{i}"] = f"=IF(ROUND(P{i}-O{i}, 2)=U{i}, 0, T{i}-P{i})"
